...
namespace DotLiquid
{
	public class Context
	{
		...
		private static object Liquidize(object obj)
		{
			...
			var list = GetAttributes(obj.GetType());
            if (list.Any())
            {
                var attr = (LiquidTypeAttribute)list.First();
		...
		private static Dictionary<Type, DotLiquid.LiquidTypeAttribute[]> attributecache = new Dictionary<Type,LiquidTypeAttribute[]>();

		public static DotLiquid.LiquidTypeAttribute[] GetAttributes(Type t)
		{
			DotLiquid.LiquidTypeAttribute[] res;
			if (!attributecache.TryGetValue(t, out res)) {
				res = (DotLiquid.LiquidTypeAttribute[])t.GetCustomAttributes(typeof(DotLiquid.LiquidTypeAttribute), false);
			  attributecache.Add(t, res);
			}
			return res;
		}
